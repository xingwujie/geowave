		...
		SpaceFillingCurve
{
	...
	private static class XElement
	{
		...
		public boolean isContained(
				MultiDimensionalNumericData window ) {
			double[] windowMins = window.getMinValuesPerDimension();
			double[] windowMaxes = window.getMaxValuesPerDimension();
			for (int i = 0; i < dimensionCount; i++) {
		...
		public XElement[] children() {
			if (children == null) {
				...
				children = new XElement[(int) Math.pow(
						2,
						dimensionCount)];
